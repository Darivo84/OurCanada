...
// require_once '/var/www/html/ourcanada'.$ext.'/check_session.php';

$baseURL = 'https://'.$_SERVER['HTTP_HOST'];

$urlRediecrtion = preg_replace('~//+~', '/', $_SERVER['REQUEST_URI']);
if($_SERVER['REQUEST_URI'] !== $urlRediecrtion){
    $urlRedic = str_replace("community/community/","community/",$baseURL.$urlRediecrtion);
    header("Location:".$urlRedic);
}
...
$pages_array=['about-us','contact-us','form','test','login','signup','password-reset','change-password','terms'];
$language_code='';
...
if(!function_exists('get_pagination_links2')){
    function get_pagination_links2($current_page, $total_pages,$url = '')
    {
        ...
        echo '<ul class="pagination">';
        $links = "<li ".$prevPageDisable."><a href='{$url}'><</a></li>";
        if ($total_pages >= 1 && $current_page <= $total_pages) {
            $links .= "<li ".$firstPage."><a href='{$url}'>1</a></li>";
            $i = max(2, $current_page - 5);
            if ($i > 2)
                $links .= " <li><a href='javascript:void(0);'>...</a></li> ";
            for (; $i < min($current_page + 6, $total_pages); $i++) {
                ...
                $links .= "<li ".$currentPageActive." ><a href='{$url}'>{$i}</a></li>";
            }
            if($total_pages > 1){
                if ($i != $total_pages)
                    $links .= " <li><a href='javascript:void(0);'>...</a></li> ";
                $links .= "<li ".$lastPage." ><a href='{$url}'>{$total_pages}</a></li>";
            }
        }
        $links .= "<li ".$nextPageDisable." ><a href='{$url}'>></a></li>";
        if($total_pages < 1){
            return '';
        }else{
            return $links.'</ul>';
