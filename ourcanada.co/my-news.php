...
if(!isset($_SESSION['user_id'])){
  header("location: ".$cms_url.'login'.$langURL);
}
$news_table = 'news_content';
if(!empty(getCurLang($langURL,true))){
  $news_table = $news_table.'_'.getCurLang($langURL,true);
}
?>
<!DOCTYPE html>
...
<meta name="keywords" content="">
    ...
    {
    ?>
    <link rel="canonical" href="https://ourcanada<?php echo $ext ?>/community/my-news">

        <?php
    }
    ?>
<!-- Title-->
<title><?= $allLabelsArray[502] ?></title>
<!-- Favicon--> 
          ...
          <div class="homepage_builder_title">
              <?php if($displayType == 'Right to Left'){ ?>
                  <?php if(!empty($_SESSION['user_id'])){?>
                      <div class="col-sm-4 mt-15" style="padding-right: 0px !important;"><a class="btn btn-login" style="float: left" href="<?php echo $cms_url ?>create-news<?= $langURL ?>"><?= $allLabelsArray[511] ?></a></div>
                  <?php }?>
                  <div class="col-sm-8" style="padding-left: 0px !important;">
                      <h2 style="float: right"> <?= $allLabelsArray[502] ?> </h2>
                      <span class="jl_hsubt"><?= $allLabelsArray[510] ?></span> </div>

              <?php } else { ?>
                  <div class="col-sm-8" style="padding-left: 0px !important;">
                      <h2> <?= $allLabelsArray[502] ?> </h2>
                      <span class="jl_hsubt"><?= $allLabelsArray[510] ?></span> </div>
                  <?php if(!empty($_SESSION['user_id'])){?>
                      <div class="col-sm-4 text-right mt-15" style="padding-right: 0px !important;"><a class="btn btn-login pull-right" href="<?php echo $cms_url ?>create-news<?= $langURL ?>"><?= $allLabelsArray[511] ?></a></div>
                  <?php }?>
              <?php } ?>
          ...
          <div class="jl_wrapper_row jl-post-block-108832">
            <div class="row">
              ...
              <?php 
                ...
                if($page > 1){
                  $query = "SELECT * FROM `".$news_table."` WHERE creator_id = '{$_SESSION['user_id']}' LIMIT ".( 12 * ($page - 1)).",12";
                }else{
                  $query = "SELECT * FROM `".$news_table."` WHERE creator_id = '{$_SESSION['user_id']}' LIMIT 0,12";
                }
...
//             echo "'".$cms_url.'uploads/gallery/'.$row['content_thumbnail']."'";
                  ...
                  <div class="post-entry-content">
                    ...
                    <span class="jl_post_meta">
                      ...
                      <span class="post-date"><i class="fa fa-clock-o"></i><?php echo time_ago($row['created_at']); ?><br><span style="margin-left: 30px;">UTC-08:00</span></span></span>
                    
                  </div>
          <a>
            
          <a class="edit" href="<?php echo $cms_url."edit-content/".$row['slug'].$langURL; ?>?news"><i class="fa fa-pencil-square-o"></i></a>
          </a>
          ...
          <?php } ?>
              ...
              <div class="col-lg-12">
                <?php 
                  $query1 = "SELECT COUNT(*) as total FROM `".$news_table."` WHERE creator_id = '{$_SESSION['user_id']}'";
                ...
                <ul class="pagination">
                  ...
                  <li <?php if(isset($_GET['page']) && $_GET['page'] == ($i + 1)){echo 'class="disabled"';} ?> style="cursor: pointer;">
                    <a <?php if(isset($_GET['page']) && $_GET['page'] == ($i + 1)){}else{?>href="<?= $cms_url.'my-news?page='.($i + 1) ?>"<?php } ?>><?= $i + 1 ?></a>
                  </li>
                <?php } ?>
                </ul>
              <?php } ?>
  ...
  function deleteContent(id){
    ...
    $("#del_conf_modal .btn-primary").click(function(){
      ...
      $.ajax({
        ...
        data:{id:content_id,table:"<?= $news_table ?>"},
        dataType: "json",
        beforeSend: function(){
          sel.children("i").show();
          ...
          if(res.success){
            ...
            $("#editor-error").text("Failed to delete content.");
          }
        },error: function(e){
          sel.children("i").hide();
          console.log(e);
