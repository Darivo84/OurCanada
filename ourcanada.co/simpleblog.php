								...
								<div class="post-2808 post type-post status-publish format-standard has-post-thumbnail hentry category-business tag-gaming tag-morning tag-relaxing" id="editor">
									...
									<div class="single_section_content box blog_large_post_style">
										<div class="jl_single_style2">
										    ...
										    <h1 style="margin-bottom: 20px !important;" class="single_post_title_main title"><span><?= $allLabelsArray[560] ?></span>
										    	<span class="editSpan" >
...
function loadGalleryImages(){
	...
	n = 0;
	$("#gallery_image_thumbnail .modal-body").load('<?= $main_app ?>load_gallery_images.php?cur='+n+'&content_type=<?= $_GET['content_type'] ?>&lang=<?= getCurLang($_GET['lang'],true); ?>');
}
...
$(".tages_list li").click(function(){
    $(this).toggleClass("active");
				...
				function CreateBlog(sel){
					...
					$("#create_blog input[name=type]").val("simple"+$("input[name=getType]").val());
                    ...
                    // 	ShowMessage("error","Image is not allows in quote!","");

                    // }else if($("input[name=content_one]").val().includes("<img")){

                    // 	ShowMessage("error","Image is not allows in content 1!","");

                    // }else if($("input[name=content_two]").val().includes("<img")){

                    // 	ShowMessage("error","Image is not allows in content 2!","");

                    // }else if($("input[name=content_three]").val().includes("<img")){

                    // 	ShowMessage("error","Image is not allows in content 3!","");

                    // }else if($("input[name=content_four]").val().includes("<img")){

                    // 	ShowMessage("error","Image is not allows in content 4!","");
                    ...
                    // }
                        ...
                        $.ajax({
							type: "POST",
							url: "<?php echo $cms_url ?>ajax.php?h=CreateUpdateSimpleBlog&lang="+$("input[name=lang]").val(),
							data: $("#create_blog").serialize(),
							dataType: "json",
							beforeSend: function(){
								sel.children("i").show();
