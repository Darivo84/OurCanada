...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    echo '<script>window.open("'.$currentTheme.'superadmin","_SELF")</script>';
}
...
$getContent = mysqli_query($conn,"SELECT * FROM ".$_GET['type']."_content".$lang_content." WHERE id = ".$_GET['id']);
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div>
                                        ...
                                        <div class="form-group row mb-4" id="video-content">
                                            ...
                                            <div class="col-lg-12" style="text-align: center; margin-bottom: 10px;">
                                                <div class="row" style="padding: 0 15px;">
                                                    <input type="url" id="youtubeVid1" value="<?= $contentRow['embed'] ?>" style="margin-bottom: 10px;" name="embed" class="form-control col-lg-10" placeholder="Add embed video link...">
                                        ...
                                        <div id="image-video-content">
                                            <div class="row">
                                                ...
                                                <div class="col-lg-12" style="text-align: center; padding: 10px 0;">
                                                    ...
                                                    ?>
                                                    <video style="width: 80%; height: 400px; background: #000; <?php if(empty($img_list)){echo "display: none;";} ?>" src="<?= $img_path ?>" controls></video>
                                                    <br>
                                                ...
                                                <div id="video-image" class="col-lg-12" style="text-align: center; margin-bottom: 10px; margin-top: 10px;">
                                                    <div class="row" style="padding: 0 15px;">
                                                        <input type="url" id="youtubeVid2" style="margin-bottom: 10px;" name="youtube_embed" value="<?php if(!empty($contentRow['embed'])){echo $contentRow['embed'];} ?>" class="form-control col-lg-10" placeholder="Add embed video link...">
                                                    ...
                                                    <iframe width="80%" height="400" src="<?php if(!empty($contentRow['embed'])){echo $contentRow['embed'];} ?>" style="margin-top: 10px; <?php if(empty($contentRow['embed'])){echo 'display: none;';} ?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                                </div>
                                            </div>
                                        </div>
                                        <?php } ?>
                                        ...
                                        <br><br>
                                        <?php } ?>
                                        <?php if(in_array($contentRow['type'], ['simpleblog','simplenews','pdf-blog','pdf-news'])){ ?>
...
<script src="https://cdn.ckeditor.com/4.16.0/full/ckeditor.js"></script>
    ...
    function CreateContent(sel){
        ...
        if(Validate()){
            ...
            form.append('table',"<?= $_GET['type'] ?>_content<?php if(isset($_GET['lang'])){echo '_'.$_GET['lang'];} ?>");
            ...
            if($("input[name=date]").val() != "" && $("input[name=time]").val() != ""){
                if($("input[name=date]").val()+" "+$("input[name=time]").val() != "<?= $contentRow['created_at'] ?>"){
            ...
            if("<?= $contentRow['type'] ?>" == "video-blog" || "<?= $contentRow['type'] ?>" == "video-news"){
                if($("#youtubeVid1").val() != ""){
                    form.append("youtube_embed",$("#youtubeVid1").val());
                }
            }else{
                if($("#youtubeVid2").val() != ""){
                    form.append("youtube_embed",$("#youtubeVid2").val());
                }
            }

            

            if($("#pdf_file").val() != "" && typeof $("#pdf_file")[0] !== 'undefined' && typeof $("#pdf_file")[0].files[0] !== 'undefined'){
            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    if(res.error){
                        ...
                        $(".prompt").text("Content update successfully."); 
                        $('html, body').animate({
                            scrollTop: 0
                        }, 500);
                        setTimeout(function(){
                            window.open('<?= $super_admin ?>admin-<?= $_GET['type'] ?>-listing','_SELF');
                        },5000);
                    }
                },
                error: function(e){
    ...
    function Validate(){
        ...
        if('<?= $contentRow['type'] ?>' == 'simpleblog' || '<?= $contentRow['type'] ?>' == 'simplenews'){
            if($("#thumbnail-image-gallery .active").length < 1){
        ...
        if(check){
            if($("#content_one").val() == "" && $("#content_two").val() == "" && $("#content_three").val() == "" && $("#content_four").val() == ""){
                ...
                if("<?= $contentRow['type'] ?>" == "video-image-<?= $_GET['type'] ?>"){
                    if(upload_gallery == "" && (upload_video == null || upload_video.files.length < 1) && $("#youtubeVid2").val() == ""){
                        $(".prompt").attr("class","prompt alert alert-danger");
                        ...
                        $(".prompt").attr("class","prompt alert alert-danger");
                        $(".prompt").text("You can not add more then 4 images.");
                        $('html, body').animate({
                            scrollTop: 0
                        }, 500);
                        check = false;
