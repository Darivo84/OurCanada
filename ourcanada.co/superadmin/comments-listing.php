        ...
        <div class="row">
          <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
              <h4 class="mb-0 font-size-18">Blogs Comments</h4>
                ...
                <div class="table-responsive">
                  <table id="categoryTable" class="table table-centered table-nowrap table-hover">
                    ...
                    <tbody>
                      ...
                      $sqlQuery = " SELECT comments.*,comments.status as c_status,comments.id as com_id,".$table_name.".title,users.username,DATE_FORMAT(comments.created_at,'%M %d,%Y %h:%i %p') as c_date FROM comments 
                      LEFT JOIN users ON comments.user_id = users.id 
                      LEFT JOIN ".$table_name." ON comments.content_id = ".$table_name.".id 
                      WHERE comments.lang = '".$langName."' 
                      ORDER BY comments.id DESC";
                      // $getCom = mysqli_query($conn,"SELECT comments.*,comments.status as c_status,comments.id as com_id,blog_content.title,users.username,DATE_FORMAT(comments.created_at,'%M %d,%Y %h:%i %p') as c_date FROM comments LEFT JOIN blog_content ON comments.content_id = blog_content.id LEFT JOIN users ON comments.user_id = users.id ORDER BY comments.id DESC");
                      $getCom = mysqli_query($conn,$sqlQuery);
                      while ($com_row = mysqli_fetch_assoc($getCom)) {
                        if(!empty($com_row['title'])){
                      ?>
                      <tr>
                        <td><?= $count_row++ ?></td>
...
<script>
  ...
  function getContent(value){
        if(value != ""){
            window.location.href = "<?php echo $currentTheme ?>superadmin/comments-listing?lang="+value;
        }else{
            window.location.href = "<?php echo $currentTheme ?>superadmin/comments-listing";
        }
    }
  ...
  function aprove(sel,id){
    $.ajax({
      type: "POST",
      url: "<?= $super_admin ?>ajax.php?h=comment_satus",
      data: {id:id},
      dataType: "json",
    ...
    function deleteComment(id){
      ...
      $("#delCommentModal .btn-danger").click(function(){
        ...
        $.ajax({
          ...
          data: {id:comment_id,table:"comments"},
          dataType: "json",
          beforeSend: function(){
            sel.children("i").show();
          },success: function(res){
    ...
    function editComment(comment,id){
      ...
      $("#editComment .btn-success").click(function(){
        ...
        $.ajax({
          ...
          data:{id:comment_id,comment:update_comment,table:"comments"},
          dataType: "json",
          beforeSend: function(){
            sel.children("i").show();
          },success: function(res){
