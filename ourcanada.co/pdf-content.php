...
<div class="jl_home_section">
              ...
              <ul>
                ...
                <?php } ?>
										    ...
										    <h1 class="single_post_title_main"><span id="title_display"><?= $getRow['title'] ?></span>
										    	<span class="editSpan">
										...
										<div style="margin-bottom: 30px;" class="urdu_arabicBTN">
											<span class="editSpan">
												...
												<div id="selected_file" style="width: 100%; text-align: left; margin-top: -50px; height: 50px;">
													<?php if(!empty($getRow['pdf_path'])){echo  $allLabelsArray[812] . $getRow['pdf_path'];} ?>
												</div>
											</span>
										</div>
...
function selectPDF(sel){
	...
	if(sel[0].files[0].type == 'application/pdf'){
		...
		// $(document).find("#selected_file").text("<?php // if(!empty($getRow['pdf_path'])){echo $getRow['pdf_path'];} ?>");
		sel.val("");
	}
}
...
$(document).ready(function(){
	...
	function UpdateContent(sel){
		...
		if(updateCheck == 0){
		...
		}else if(pdfFile.val() != "" && pdfFile[0].files[0].type != "application/pdf"){
			...
			$.ajax({
				type: "POST",
				url: "<?= $cms_url ?>ajax.php?h=CreateUpdatePdfBlog&lang=<?= getCurLang($langURL,true); ?>",
				data: formData,
