...
if ($_GET['h'] == 'getQuestion') {
    ...
    if (mysqli_num_rows($select) > 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if($_POST['id']==117 && $_POST['pid']==115)
            {
                $log['submit_button_view']=1;
                user_logs($log);
            }
...
if ($_GET['h'] == 'submitForm') {
    ...
    $duties_org = array_map('trim', $duties_org);
    $duties = array_map('trim', $duties);
    $positions_org = array_map('trim', $positions_org);
    $positions = array_map('trim', $positions);

    array_walk($positions, 'custom_replace');
    array_walk($duties, 'custom_replace');

    $positions=array_map('trim',$positions);
    $duties=array_map('trim',$duties);

    $positions=array_unique($positions);
    $duties=array_unique($duties);

    $positions=array_filter($positions,'strlen');
    $duties=array_filter($duties,'strlen');
    ...
    if ($_POST['endCase'] === false || $_POST['endCase'] === 'false') {
        ...
        if ($_POST['scoreID'] != '') {
            ...
            if($empty==sizeof($user_noc))
            {
                unset($user_noc);
            }
            $empty=0;
            foreach ($spouse_noc as $k)
            {
                if(empty($k))
                {
                    $empty++;
            ...
            for ($i = 0; $i < sizeof($user_noc); $i++) {
                ...
                if (sizeof($job) > 0 && $job[0] != '') {
                    ...
                    foreach ($job as $j) {
                        ...
                        else if(strpos($j,'(RN)')!==false)
                        {
                            $j2=explode('(',$j);
                            $noc = mysqli_query($conn, "select * from noc2 where job_position like '%$j2[0]%'");
                            if (mysqli_num_rows($noc) > 0 && $count == 0) {

                                while ($nr = mysqli_fetch_assoc($noc)) {
                                    $jp = explode('*', $nr['job_position']);
                                    $jp=array_map('trim',$jp);
                                    array_walk($jp, 'custom_replace');
                                    $jp=array_filter($jp,'strlen');
                                    $j=trim($j);
        ...
        for ($o = 0; $o < sizeof($quesArray); $o++) {
            if(!empty($ansArray[$o])) {
                if (strpos($quesArray[$o], ' job duties') !== false) {
                    ...
                    $index = array_search(trim($ansArray[$o]), $duties);
...
//    $_POST['n']['status'] = 0;
    ...
    foreach ($automationScriptEmails as $k => $v) {
        $temp = '';
        if ($v['country']=='' || $v['country'] == $mainQuesArray[571] || $v['country'] == $mainQuesArray[43] || $v['country'] == $mainQuesArray[45] || $v['country'] == $subQuesArray[70] || $v['country'] == $subQuesArray[71] || $v['country'] == $subQuesArray[11622]) {
...
//    {
//
//        foreach ($nocUser_array as $k=>$value)
//        {
////            echo isset($k).'<br>';
//            echo $k.'-'.empty($k).'<br>';
////            echo sizeof($k).'<br>';
...
//    }
    ...
    $total_user_exp=0.0;
    $total_spouse_exp=0.0;



    for ($j = 0; $j < sizeof($nocUser_array); $j++) {
        $total_user_exp+=$nocUser_array[$j]['experience'];

    }
    for ($j = 0; $j < sizeof($nocSpouse_array); $j++) {
        $total_spouse_exp+=$nocSpouse_array[$j]['experience'];
    ...
    while ($r = mysqli_fetch_assoc($select)) {
        ...
        for ($i = 0; $i < $len; $i++)
        {
            ...
            if ($r['previous_years'] != '') {
                ...
                if($myArray[$i]['experience'] > $previous_years)
                {
                    if($last_year>=$start_year && $current_year<=$end_year)
                    {
                        $prev = true;
                        $valCount++;
                    }
...
//                        $colCheck = true;
//                    }
//                    if (check_cond($myArray[$p]['experience'], $r['operator'], $r['value']) && !$experience) {
//                        $experience = true;
//                        $valCount++;
//                    }
//                } else
                    {
                        // copy above else
                        $skillNOC = explode(',', $myArray[$p]['noc']);
                        $skillNOC=array_filter($skillNOC);
                        foreach ($skillNOC as $sn) {
                            if(array_search(substr($sn, 0, 2),$snoc) > -1)
                            {
                                $e += (float)$myArray[$p]['experience'];
                                break;
                            }
                        }
                        $exp = $e;
            ...
            if ((empty($r['skill']) && empty($r['noc_number']) && empty($r['country']) && (empty($r['province']) || $r['province']==',') && (empty($r['region']) || $r['region']==','))) {
                if ($r['noc_type'] == 'user') {
                    $exp=$total_user_exp;
                }
                else {
                    $exp=$total_spouse_exp;
                }
            ...
            if ($r['value'] != '' && $r['same_job'] != '1') {
                ...
                if (check_cond($exp, $r['operator'], $r['value'])) {
                    $experience = true;
                    $valCount++;
                }
            }
...
function autoSave($sformId,$form_email)
{
    global $conn;
...
//    echo $data['email']."<br>";
    ...
    $data2['html_data']=$_POST['formHtml'];
    $data2['lang']=$_GET['Lang'];
    ...
    if(isset($_SESSION['user_id']))
    {
        ...
        if($user_row['role']==1 || $user_row['role']=='1')
        {
            ...
            else
            {
                ...
                $autoSaveID=mysqli_insert_id($conn);
            }
            $data2['auto_save_id']=$autoSaveID;
            $Z = db_pair_str2($data2);
            $insert=mysqli_query($conn,"insert into auto_save_form_logs set $Z");
...
function user_logs($data)
{
    ...
    if(isset($_SESSION['user_id']))
    {
        $select=mysqli_query($conn,"select * from user_logs where user_id=$user_id and session_id='$session_id' and ip_address='$ip_address'");
    }
    else
    {
        $select=mysqli_query($conn,"select * from user_logs where session_id='$session_id' and ip_address='$ip_address'");
    }
    if(mysqli_num_rows($select) > 0)
    {
        $row=mysqli_fetch_assoc($select);
        $id=$row['id'];
        $query=mysqli_query($conn,"update user_logs set $T where id=$id");
    }
    else
    {
        $query=mysqli_query($conn,"insert into user_logs set $T ");
    }
}
